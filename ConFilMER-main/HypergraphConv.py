...
class HypergraphConv(MessagePassing):
    ...
    def forward(self, x, hyperedge_index,
        ...
        Args:
            ...
            hyperedge_index (LongTensor): [2, E] where [0]=Nodes, [1]=Edges
        """
        ...
        # Node Normalization D
        D = scatter_add(hyperedge_weight[hyperedge_index[1]],
                        hyperedge_index[0], dim=0, dim_size=num_nodes)
        D = 1.0 / D
        D[D == float("inf")] = 0
        
        # Edge Normalization B
        ...
        # 1. 节点 -> 超边 (Node to Edge)
        # 必须使用 source_to_target (0 -> 1)，此时 Source 是节点
        # 输出维度应为 num_edges
        self.flow = 'source_to_target'
        out = self.propagate(hyperedge_index, x=x, norm=B, alpha=alpha,
                             size=(num_nodes, num_edges))
        
        # 2. 超边 -> 节点 (Edge to Node)
        # 必须使用 target_to_source (1 -> 0)，此时 Source 是超边，Target 是节点
        # 输出维度应回归到 num_nodes
